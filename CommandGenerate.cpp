...
void save_nonces(unsigned int nonceSize, std::ofstream *out, unsigned char *bufferCpu, std::string path) {
  ...
  std::cout << "done saving to " << path << std::endl;
  std::cout.flush();
	...
	try {
		...
		for (unsigned long long nonce_ordinal = 0; nonce_ordinal < maxNonceNumber; nonce_ordinal += staggerSize) {
			for (unsigned int jobnum = 0; jobnum < paths.size(); jobnum += 1) {
				...
				save_threads[jobnum] = std::async(std::launch::async, save_nonces, nonceSize, out_files[jobnum], buffersCpu[jobnum], paths[jobnum]);
			}
		}
